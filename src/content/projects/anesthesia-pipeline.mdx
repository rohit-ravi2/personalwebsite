...
General anesthesia is driven by **coordinated partial perturbation across a multi-class target ensemble** (SNARE release, Complex I, K2P channels, nAChR, NCA leak, GABA-A potentiation), not a single-target binding event. Mutants in any one target give partial phenotypes; non-immobilizers like hexafluoroethane lack engagement at the high-magnitude classes the architecture requires.

**What this project earned, stated once:** within-organism volatile EC50 prediction ~3× tighter than Meyer-Overton on held-out isoflurane across worm, fly and mouse, with one free parameter per organism; mutant magnitude predictions inside published literature bands; and a 4–6 mechanism-class minimum sufficient subset in which SNARE-or-Complex-I is universal (25/25 passing subsets).

**What it does not establish.** A 2026-06-12 preregistered, accept-either-way re-examination (see *Adversarial close-out*) proved the V1 operator is **rank-2** — every anesthetic and genotype profile collapses to a uniform current scalar plus one global synaptic gain, so the network *cannot by construction* distinguish which targets are engaged from the aggregate magnitude of two coordinates. Cell-type targeting, tested on a rank-lifted substrate built specifically to make it testable, returns a **null** on worm (46th percentile). The immobilization readout is **demoted to a network statistic**, not validated behavioral quiescence. Whether the *specific* target identities are load-bearing, rather than one magnitude-matched ensemble among many, is **not established here**.

**Retraction (2026-09-21).** Earlier versions of this page headlined a *worm-anchor-overfit / fly-uniquely-special / mouse-magnitude-driven* trichotomy. That reading came from a null-model bug — it added a phantom 50 pA SNARE current the operator never applies, and never matched the actual synaptic-gain coordinate. Corrected percentiles are **worm 28% / fly 26% / mouse 38%**: worm ≈ fly, both modestly significant, mouse marginal. Sections asserting the old reading have been removed; the historical record is preserved in a collapsed block near the foot of this page.
...
**Net.** The core thesis survives — a multi-class quorum (4–6 classes; SNARE-or-Complex-I universal) reproduces MAC, and anesthetics discriminatively engage a biologically-motivated target panel. But the project's own substrate, even after the rank-lift built to test it, does **not** establish that the *specific* cell-type-resolved target identities are load-bearing rather than one magnitude-matched ensemble.

## The successor result (2026-09): a cross-state mechanism map, and an asymmetry nobody has asked about

The "Paper 2" question this project posed — *does anesthetic-induced quiescence act on the machinery of natural reduced-activity states?* — was blocked here on the demoted readout. It was answered by a different method: a frozen mechanism-repertoire × state map built directly from primary literature, with the search instrument itself calibrated against a measured recall standard.

**Scope:** 12 scored (state, species) rows × 12 mechanism classes defined by *physical action* = 144 cells, spanning sleep, anesthesia, dauer, L1 starvation arrest, cryptobiosis, and both seasonal hibernation and mouse daily torpor. Three-valued evidence logic (`TESTED_POSITIVE` / `TESTED_NEGATIVE` / `NOT_TESTED`, never collapsed to a binary), an orthogonal causal-role axis, and three phenotype axes that are never merged into one "depth" score. 162 verified PMIDs, every title fetched rather than recalled. Measured internal retrieval recall 1.00 on every row.

| state production | rows | PRODUCES | ENTRY | MAINTAINS | PROTECTS | BUILDS | EXIT |
|---|---|---|---|---|---|---|---|
| endogenous (sleep, dauer, L1 arrest, hibernation) | 7 | **17** | 20 | 17 | 4 | 5 | **2** |
| physically imposed (cryptobiosis) | 1 | 0 | 2 | 0 | 2 | 1 | 0 |
| exogenous agent (anesthesia) | 4 | **0** | 18 | 3 | 3 | 0 | **5** |

**No mechanism has ever been shown to *produce* the anesthetized state.** Exactly zero, across four rows and 25 populated cells, against 17 in the endogenous states. Not a coverage artifact: the most completely scored row in the table — mouse volatile anesthesia at 11/12 cells — is one of the four.

**The matching claim about exit was falsified, deliberately.** An earlier version of this table had zero `ENABLES_EXIT` across every endogenous state. Hibernation was then scored *specifically because* it was the state most likely to break that, and it did: intracerebroventricular prostaglandin E₂ provokes arousal from an established hibernation bout ([PMID 11893609](https://pubmed.ncbi.nlm.nih.gov/11893609/)), and central adenosine antagonism reverses established torpor ([PMID 22785425](https://pubmed.ncbi.nlm.nih.gov/22785425/)). The second is scored the same way as caffeine-accelerated emergence from anesthesia because it is the same pharmacology delivered the same way — scoring them differently would have preserved the zero and been indefensible.

**What replaces it is sharper.** All seven endogenous rows share a production mode; only hibernation has an exit literature. The discriminating variable is not biology but cost: interbout arousals consume **60–80% of a ground squirrel's entire winter energy budget**, and their function is, in the source's own words, "unknown and disputed."

> Exit is studied where exit is costly. Production is studied where production is not already explained by the intervention.

Anesthesia has an exit literature because emergence is a clinical problem. Hibernation has one because arousal is an energetic problem. Sleep has none because waking up is taken for granted. Nothing produces the anesthetized state because the drug already answers the question. The shape of the literature follows the shape of the question — and hibernation supplies the natural experiment: two groups matched on production mode, differing in exit cost, differing in exactly the predicted direction.

**A convergence nobody looked for.** Two states, two sufficiency tests, same structure. Central A1-agonist activation in mouse produces profound hypothermia that is *not torpor* — it separates from the real state on temperature kinetics, heart rate, shivering, arrhythmia, recovery rate and transcriptional signature ([PMID 28378088](https://pubmed.ncbi.nlm.nih.gov/28378088/)). Chemogenetic reactivation of anesthesia-activated ensembles produces immobility, discoordination and anti-nociception but *not* loss of righting ([PMID 41573848](https://pubmed.ncbi.nlm.nih.gov/41573848/)). Independent literatures, independent methods: **the components of a reduced-activity state can be produced separately, and producing some of them does not assemble the state.** Neither paper cites the other.

**The Paper-2 question, answered.** Caspase ablation of parafacial-zone GABAergic neurons in mouse ([PMID 40240218](https://pubmed.ncbi.nlm.nih.gov/40240218/)) confers durable behavioral resistance to *both* isoflurane and sevoflurane — ~50% higher likelihood of intact righting, dose-graded against surviving neuron count — and in the *same animals, in the same paper*, leaves NREM sleep amount, wakefulness and homeostatic drive unchanged with 70–80% of the population destroyed. A node of the sleep-promoting circuitry is **required for the drug-induced state and dispensable for the endogenous one.** Anesthesia co-opting sleep circuitry survives as a hypothesis, with a constraint it did not have before: **the co-option is not symmetric.**

The same paper carries a second dissociation its authors state explicitly — the behavioral endpoint moved and the EEG signature did not. A single collapsed "anesthetic depth" score would have erased both findings.

**The closest anyone has come to the missing experiment, and it came apart.** [PMID 41573848](https://pubmed.ncbi.nlm.nih.gov/41573848/) (preprint) captured isoflurane-activated neurons brain-wide with a Fos-driven DREADD system and reactivated them without anesthetic. Immobility, discoordination, anti-nociception and increased anesthetic sensitivity all appeared. **Loss of consciousness, measured by righting reflex, did not.** The gap above is therefore not "unattempted" — it is "attempted at ensemble level, and the components of the anesthetic state dissociated under the attempt."

**Agent specificity, established four independent times.** Anesthetics are routinely discussed as a class; the manipulation evidence says they are not one. Ndufs4 knockout mice are 2.5–3× hypersensitive to isoflurane and halothane, 2× to propofol, and **resistant to ketamine**. Stomatin knockout shifts diethyl ether sensitivity by 12% and leaves halothane and isoflurane unchanged (a measured null with a positive internal control). EAAT3 knockout lowers isoflurane *hypnosis* threshold while leaving isoflurane *immobility* untouched. N-type Ca²⁺ channel knockout is *more* halothane-sensitive and *less* propofol-sensitive — oppositely affected, in one genotype. **"Anesthetic sensitivity" is not one quantity; it is at least agent-specific and, within one agent, endpoint-specific.**

**The most confident claim in the dormancy literature is unexamined.** A prediction frozen *before* any scoring said that for the deep states, the membrane-excitability classes would come back `NOT_TESTED` rather than `TESTED_NEGATIVE` — and called that the single most valuable possible output. It holds, now on four deep states: dauer, cryptobiosis, seasonal hibernation and mouse daily torpor are all empty across the entire merged excitability block, at recall 1.00. "Dormancy reduces neural excitability" is asserted constantly and has not been tested at the mechanism level in any of them.

**Method result, portable beyond this project.** The literature-search protocol used in every earlier run had a **measured recall of 0.05** — NCBI eutils ANDs bare terms, so concatenated multi-concept queries demanded every term appear, and the founding worm anesthesia-genetics paper returned zero hits. Rebuilding it as boolean OR-groups with calibrated retrieval depth took measured internal recall to **1.00 (112/112)**, on a hold-out target that grew 170% during calibration. Two results generalize: broadening query vocabulary at fixed retrieval depth *costs* recall, and every residual miss must be attributed to vocabulary-versus-depth by direct diagnostic before either is changed. **A null is only as interpretable as its instrument's measured recall** — which is why 53 of 120 cells can be reported as "searched and empty" at all.
...
**V7 Sub-Q2 + Sub-Q1 sharpen the conserved-target claim and narrow it further.** The architecture requires a 4–6 class minimum sufficient subset per organism (worm 5, mouse 4, fly 6); no 1-/2-/3-class subset passes anywhere. SNARE OR Complex I appears in 100% of all-stages-passing subsets across all three organisms (25/25, exceeding the pre-registered 75% threshold). Pre-registered P2 and P5 were **falsified** in the brittle-to-low-subset-counts direction. On the corrected random-ensemble null (Match #2b), all three organisms sit in a narrow band — **worm 28% / fly 26% / mouse 38%** — a modest, significant conserved-profile advantage in worm and fly and a marginal one in mouse. A mean-field theorem (per-neuron input CV² ∝ 1/K, slope −1.03, R² 0.999) shows the compression toward the median is a *derived* consequence of graph structure rather than a failed test.
...
> A literature-anchored multi-class Hill perturbation table, integrated through a substrate-agnostic LIF integrator, recovers held-out volatile EC50s ~3× tighter than Meyer-Overton across worm, fly, and mouse with one free parameter calibrated per organism. The architecture requires a 4–6 class minimum sufficient subset per organism (V7 Sub-Q2); SNARE or Complex I is universal across all 25 all-stages-passing subsets. Against a corrected two-coordinate magnitude-matched null, the conserved profile shows a modest advantage in worm (28%) and fly (26%) and a marginal one in mouse (38%). Cell-type-resolved target specificity, tested on a substrate rank-lifted expressly to make it testable, is **null** on worm (46th percentile) and untestable on fly. Calibration generalizes anchor-to-anchor at grid resolution (V5 M4). Eger non-immobilizer specificity is entailed by correct volatile EC50 prediction (V7 Sub-Q2 stage attrition), not an independent capability.

The architecture is largely substrate-agnostic. The mechanism map is what does the work, at the 4–6 class quorum scale. Cross-organism MAC similarity is consistent with Meyer-Overton (lipid biophysics) and not strong evidence of conserved targets on its own. The architecture's distinctive contribution is **within-organism volatile EC50 precision and mutant magnitude prediction**. Whether the *specific* target identities are load-bearing rather than one magnitude-matched ensemble among many is not established on this substrate, in any of the three organisms.
...
<details>
<summary className="cursor-pointer text-sm font-medium text-zinc-700 mt-6">Historical record — V7 adversarial controls as originally reported (superseded 2026-06-12; retained for provenance)</summary>

**Read this block as history, not as current claims.** The Sub-Q1 percentiles below came from a null model that mismatched the rank-2 operator. The corrected figures are worm 28% / fly 26% / mouse 38%, in the close-out at the top of this page. The subset-search results (Sub-Q2), sensitivity (M3) and cross-calibration (M4) in this block are unaffected and still stand.
...
**Paper 2 was answered, by a different method.** The planned bridge experiment — anesthetic-induced quiescence versus natural reduced-activity states, tested inside this simulator — stayed blocked on the demoted readout and was never run here. The question was instead answered at literature scale by the cross-state mechanism map described near the top of this page, which is independent of this substrate and of the readout that blocked it. Its answer: the two states share circuit nodes, but the dependency is **asymmetric** — a node can be required for the drug-induced state and dispensable for the endogenous one.

**Framework claims about the thermodynamic necessity of reversible reduced-activity states remain out of scope**, and a separate audit has since sharpened why. The estimator the field uses for this — coarse-grained forward/reverse KL on neural recordings, reported as "entropy production" or "arrow of time" — requires Markovianity, stationarity, observable reversibility, structure-preserving lumping, and no hidden drive. All five fail for calcium imaging, and the failure is measurable: on 38 whole-brain *C. elegans* recordings the real irreversibility (0.0133 nats/frame) sits **below the indicator-kinetics floor** (0.0552) and barely above phase-randomized surrogates (0.0120), with 0/38 animals above the floor. The quantity is a statistic of the observed dynamics, not entropy production and not a bound on it.

**A controlled follow-up (2026-09-21) found two confounds, with opposite signs.** Latent processes with analytically known ground truth were measured with the identical estimator. The prediction I went in with — that a causal asymmetric filter manufactures irreversibility — was **falsified**: on a smooth reversible source, causal filtering *lowers* the estimate.

What actually drives it is **finite-sample bias that scales with the latent timescale**. On a provably reversible process with true entropy production of exactly zero, the estimator reads 0.1286 for fast dynamics and 0.0075 for slow — **17× across the range, with zero real dissipation at either end** — and the effect decays with recording length, confirming it as finite-sample. Anaesthesia and deep sleep slow neural dynamics, so this confound carries the *same sign* as every published wake > anaesthesia result. A second confound, causal-filter asymmetry in the event-like regime, runs the *opposite* way: it inflates the anaesthetised state. The two compete rather than add.

The estimator's positive control passes — a rotational Ornstein–Uhlenbeck process with genuinely broken detailed balance reads ~8× the reversible floor — so this is not an estimator that returns noise on everything.

Stated narrowly, and this is the whole claim: **the estimator has a documented, sign-known, uncontrolled dependence on exactly the signal properties that define the states being compared.** No study in that literature reports either control that would separate confound from effect — a timescale-matched reversible surrogate, or a symmetric-filter comparison. Both are cheap and run on data already published. (A 1996 *Biological Cybernetics* paper made the same argument form against chaos metrics in EEG; the objection was never answered for irreversibility metrics.)
...
- **Cross-state mechanism map complete (2026-09-17, extended 2026-09-21).** 12 rows × 12 mechanism classes = 144 cells, all terminally disposed; 162 verified PMIDs, 0 unresolved; measured internal retrieval recall **1.00 (120/120)** on every row; schema and completion validators both exit 0. This is the successor line and it is independent of the simulator substrate.
- **Hibernation scored and the mandated class-coarsening run (2026-09-21).** Hibernation was scored expressly to attack the headline and broke half of it; the coarsening merged classes 1–4 and reversed nothing. Both were the project's top two self-declared debts.
- **Page corrected (2026-09-21).** The superseded worm/fly/mouse trichotomy was removed from the live claims and preserved in a collapsed historical block.
...
- **Open work, simulator line**: re-ground the behavioral readout (P17 demote → re-validate against a held-out target); build a *Drosophila* cell-type-expression atlas to make fly Match #3 testable — Fly Cell Atlas and FlyWire make this buildable rather than data-gated; write the P16 held-out structure→activity scoring loop (strict shuffle + fast gates done, heavy path stubbed); regenerate Wave-P occupancy magnitudes under the corrected reference frame (P11); per-mutant Complex I factor refinement (*nduf-6 / ndus-8 / nuo-1* share an estimate).
- **Open work, mechanism-map line**: measure inter-rater reliability on causal-role assignment — now the top methodological debt; add a thermal phenotype axis; commit a stress-induced-sleep row; score the remaining 7 of 19 committed rows.
