...
Honest calibration notes are in the component's "Sources & attribution" fold. v1.5 adds a per-neuron affine distribution calibration that matches Brian2 synthetic-calcium moments to the Atanas ΔF/F training distribution — biological face-validity improved (osmotic shock now triggers the expected ASH avoidance cascade of reversal + omega + pirouette; food triggers the expected quiescence bias). What remains outstanding: classifier joint/correlation structure differs between Brian2 and Atanas even after moment-matching, so event-specific threshold calibration is still empirical. Full fix is retraining the classifier bank on Brian2-derived synthetic calcium with matched behavioral labels. Per-edge receptor mapping beyond the 22 sign overrides is also still v2 work.
