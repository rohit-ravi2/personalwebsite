...
import { CelegansDashboard } from "../../components/react/CelegansDashboard";
...
<CelegansDashboard client:only="react" />
...
Full Tier 1 stack available via `ClosedLoopEnv(brain_class="graded", environment=…)`. Shipped demo scenarios remain on the v3 LIF brain until v3.3 re-calibration (classifier bank retrained on graded-derived inputs, modulation strengths re-tuned for the different network excitation profile under graded dynamics). Next is a v3.3 ensemble audit against the new stack — the AVA/Chalfie reproduction that held under LIF should transfer, and chemotaxis-index becomes a new validation target.
